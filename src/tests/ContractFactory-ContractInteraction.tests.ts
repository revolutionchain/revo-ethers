...
describe("RevoContractFactory", function () {
    it("should deploy correctly given the deployer has enough REVO to cover gas", async function () {
        const simpleStore = new RevoContractFactory(ABI, BYTECODE, signer);
    ...
    it("can be connected to a RevoWallet signer.", async function () {
        const simpleStore = new RevoContractFactory(ABI, BYTECODE);
    ...
    it("should reject if the deployer tries sending a value", async function () {
        ...
        try {
            ...
            expect(err.reason).to.equal("You cannot send REVO while deploying a contract. Try deploying again without a value.")
        }
    });
    it("should fail as the deployer has no UTXOs to spend", async function () {
        const simpleStore = new RevoContractFactory(ABI, BYTECODE, signerNoRevo);
        try {
            await simpleStore.deploy({
                gasLimit: "0x2dc6c0", gasPrice: "0x190"
            });
