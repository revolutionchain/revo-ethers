...
import {
    configureRevoAddressGeneration,
...
export interface RevoWalletOptions {
    ...
    // this allows sent transactions to be included in a block and show up in revod apis
...
export class RevoWallet extends IntermediateWallet {
    ...
    private opts: RevoWalletOptions;
    private readonly revoProvider?: RevoProvider;
    ...
    constructor(privateKey: any, provider?: any, opts?: RevoWalletOptions) {
        ...
        this.opts = opts || {} as RevoWalletOptions;
    }

    protected async serializeTransaction(utxos: Array<any>, neededAmount: string, tx: RevoTransactionRequest, transactionType: number, opts?: SerializeOptions): Promise<string> {
     ...
     */
    async signTransaction(transaction: RevoTransactionRequest): Promise<string> {
        ...
        if (transactionType === GLOBAL_VARS.DEPLOY_ERROR) {
            return logger.throwError(
                "You cannot send REVO while deploying a contract. Try deploying again without a value.",
